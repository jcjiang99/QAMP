...
from models.qamp_ct import FewShotSeg
...
def main(_run, _config, _log):
    ...
    criterion_b = BoundaryLoss(theta0=3, theta=5)
    ...
    log_loss = {'total_loss': 0, 'query_loss': 0, 'align_loss': 0, 'aux_loss': 0, 'b_loss': 0}
    ...
    for sub_epoch in range(n_sub_epochs):
        ...
        for _, sample in enumerate(train_loader):
            ...
            query_pred, align_loss, aux_loss, b_loss = model(support_images, support_fg_mask, query_images, train=True)
            aux_loss = 0.3 * aux_loss
            b_loss += criterion_b(query_pred, query_labels)
            b_loss *= 0.8
            ...
            loss = query_loss + align_loss + aux_loss + b_loss
            ...
            b_loss = b_loss.detach().data.cpu().numpy()
            ...
            if (i_iter + 1) % _config['print_interval'] == 0:
                ...
                _log.info(f'step {i_iter + 1}: total_loss: {total_loss}, query_loss: {query_loss}, aux_loss: {aux_loss}'
                          f', b_loss: {b_loss}, align_loss: {align_loss}')
